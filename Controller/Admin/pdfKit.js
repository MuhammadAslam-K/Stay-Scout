...
async function adminRevenue(req, res) {
    ...
    try {
        ...
        }).populate("user").populate('hotel').populate('owner');
        ...
        const table = {
            headers: ['User Name', 'Hotel Name', 'Owner Name', 'Check-in Date', 'Check-out Date', 'Admin Revenue', 'Owner Revenue', 'Amount'],
            rows: bookings.map((booking) => [
                ...
                `Rs:${booking.adminAmount.toFixed(2)}`,
        ...
        const totalAdminRevenue = bookings.reduce((sum, booking) => sum + booking.adminAmount, 0);
        ...
        const totalRow = ['Total Admin Revenue:', '', '', '', '', `Rs:${totalAdminRevenue.toFixed(2)}`, `Rs:${totalOwnerRevenue.toFixed(2)}`, ''];
        if (rowCount >= rowsPerPage) {
